...
\newcommand{\subtitulo}{Programación y Optimización de Código en GPU}
...
\[
out[i] = \sum_{j=0}^{N-1} a[i] \cdot b[j]
\]
...
\[
out[i] = a[i] \cdot \left(\sum_{j=0}^{N-1} b[j]\right)
\]  
...
aún hay margen de mejora, pero estas se escapan de mis capacidades actuales. En general, se han
realizado más experimentos y optimizaciones que se han omitido por brevedad. También se dispone 
de todos los datos de las múltiples ejecuciones y configuraciones probadas, que se pueden proporcionar si es necesario. 
